...
layout = html.Div([
    dbc.Row([
        dbc.Col([
            ...
            html.Br(),
            html.Div(id='boy-brand-list-table')
        ], align="start", width={'size':4}),

        dbc.Col(
            dcc.Graph(id='boy-brand-list-graph')
        , width={'size':8})
    ], justify="around")
])
...
@app.callback(Output('boy-brand-list-table','children'),
             [Input('timeframe-dropdown','value')])
def update_table(value):
        final_df = db_utility.brand_timeframe(value,'boy')
        return html.Div([
...
@app.callback(Output('boy-brand-list-graph','figure'),
            [Input('timeframe-dropdown','value')])
def update_graph(value):
    return {'data':graph_traces.timeframe_brand_traces(value,'boy'),
