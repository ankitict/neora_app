...
layout = html.Div([
    dbc.Row([
        dbc.Col([
            ...
            html.Br(),
            html.Div(id='female-brand-list-table')
        ], align="start", width={'size':4}),

        dbc.Col(
            dcc.Graph(id='female-brand-list-graph')
        , width={'size':8})
    ], justify="around")
])
...
@app.callback(Output('female-brand-list-table','children'),
             [Input('timeframe-dropdown','value')])
def update_table(value):
        final_df = db_utility.brand_timeframe(value,'female')
        return html.Div([
...
@app.callback(Output('female-brand-list-graph','figure'),
            [Input('timeframe-dropdown','value')])
def update_graph(value):
    return {'data':graph_traces.timeframe_brand_traces(value,'female'),
