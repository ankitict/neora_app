...
layout = html.Div([
    dbc.Row([
        dbc.Col([
            ...
            html.Br(),
            html.Div(id='girl-brand-list-table')
        ], align="start", width={'size':4}),

        dbc.Col(
            dcc.Graph(id='girl-brand-list-graph')
        , width={'size':8})
    ], justify="around")
])
...
@app.callback(Output('girl-brand-list-table','children'),
             [Input('timeframe-dropdown','value')])
def update_table(value):
        final_df = db_utility.brand_timeframe(value,'girl')
        return html.Div([
...
@app.callback(Output('girl-brand-list-graph','figure'),
            [Input('timeframe-dropdown','value')])
def update_graph(value):
    return {'data':graph_traces.timeframe_brand_traces(value,'girl'),
